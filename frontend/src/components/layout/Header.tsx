...
function operatorInitials(): string {
    const email = getOperator()?.email ?? ''
    const local = email.split('@')[0]
    return (local.slice(0, 2) || 'OP').toUpperCase()
}
        ...
        <header className="h-[72px] border-b border-border flex items-center justify-between px-6 lg:px-8 bg-white/80 backdrop-blur-xl sticky top-0 z-30">
            <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                    ...
                    <p className="text-[11px] text-ink-muted font-medium">Real-time field inspections &amp; supplement claims</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="hidden sm:flex items-center gap-2 bg-brand-muted border border-brand/15 px-4 py-2 rounded-xl">
                    <Icon icon="solar:clipboard-list-bold" width="16" className="text-brand" />
                    <span className="text-xs font-semibold text-brand">{leadsCount} inspections</span>
                </div>
